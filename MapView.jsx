...
import { getDominantPractice, PRACTICE_COLORS, INSTITUTION_ICONS, getFreeTime } from './simulation';
import './MapView.css';

const INST_SHAPES = {
  work:             'M -9,-9 L 9,-9 L 9,9 L -9,9 Z',
  church:           'M 0,-11 L 9,4 L -9,4 Z',
  club:             'M 0,-10 L 3,-3 L 10,-3 L 5,2 L 7,10 L 0,5 L -7,10 L -5,2 L -10,-3 L -3,-3 Z',
  education:        'M 0,-10 L 10,5 L -10,5 Z',
  political_org:    'M 0,-10 L 9,-4 L 9,5 L -9,5 L -9,-4 Z',
  community_center: 'M -8,-5 L 0,-11 L 8,-5 L 8,8 L -8,8 Z',
};
...
export default function MapView({ sim, selectedAgent, onAgentClick, onMapClick }) {
  ...
  for (const [aId, nbrs] of adjacency) {
    for (const bId of nbrs) {
      if (bId > aId && edgeCount < maxEdges) { edges.push([aId, bId]); edgeCount++; }
    }
  }
  ...
  return (
    <div className="map-wrap">
      ...
      <svg ref={svgRef} className="map-svg" width={dims.w} height={dims.h} onClick={onMapClick}>
        <defs>
          <filter id="glow">
        ...
        {Object.entries(institutions).map(([name, inst]) => {
          ...
          return (
            <circle key={`halo-${name}`}
              cx={px(inst.position[0])} cy={py(inst.position[1])} r={r}
              fill={color} fillOpacity={hoverInst === name ? 0.10 : 0.04}
              stroke={color} strokeOpacity={0.15} strokeWidth={1} strokeDasharray="4 4"
        ...
        <g className="edges" opacity={selectedAgent ? 0.12 : 0.18}>
          {edges.map(([a, b]) => (
            <line key={`e-${a}-${b}`}
              ...
              stroke="#0f1117" strokeWidth={0.8} />
          ))}
        </g>

        {selectedAgent && [...selectedNeighbors].map(nbrId => (
          <line key={`sel-e-${nbrId}`}
        ...
        {agents.map(agent => {
          ...
          return (
            <g key={agent.id}
              ...
              {isSel && <circle r={r+4} fill="none" stroke={color} strokeWidth={2} opacity={0.5} filter="url(#glow)" />}
              <circle r={r} fill={color} fillOpacity={dimmed ? 0.18 : 1}
                stroke={isSel ? 'white' : 'rgba(255,255,255,0.6)'} strokeWidth={isSel ? 2 : 1}
        ...
        {Object.entries(institutions).map(([name, inst]) => {
          ...
          return (
            <g key={name} transform={`translate(${x},${y})`}
              ...
              <path d={shape} fill={color} stroke="white" strokeWidth={isHov ? 2.5 : 1.5}
                transform="scale(1.3)" filter={isHov ? 'url(#glow)' : undefined}
                style={{ transition: 'transform 0.15s' }} />
              <circle cx={10} cy={-10} r={8} fill="white" stroke={color} strokeWidth={1.5} />
              <text x={10} y={-6} textAnchor="middle" fontSize={8} fill={color} fontWeight="600">
                {inst.members.size}
              </text>
              <text y={20} textAnchor="middle" fontSize={9} fill="var(--ink)"
...
function Tooltip({ tooltip, institutions }) {
  if (tooltip.type === 'agent') {
    const { agent } = tooltip;
    const dom = getDominantPractice(agent, institutions);
    const alloc = Object.entries(agent.timeAllocation).sort((a, b) => b[1] - a[1]).slice(0, 4);
    return (
      <div className="map-tooltip">
        <div className="tooltip-header">
          <span className="tooltip-dot" style={{ background: PRACTICE_COLORS[dom] || '#999' }} />
          Agent {agent.id}
        </div>
  ...
  if (tooltip.type === 'inst') {
    ...
    return (
      <div className="map-tooltip">
        <div className="tooltip-header">
          <span>{INSTITUTION_ICONS[inst.practiceType] || '🏛'}</span> {inst.name}
        </div>
        <div className="tooltip-body">
          <div className="tooltip-row"><span>Members</span><span className="tooltip-val">{inst.members.size} / {inst.size}</span></div>
          <div className="tooltip-row"><span>Type</span><span className="tooltip-val">{inst.practiceType.replace('_',' ')}</span></div>
          {inst.moneyCostPerHour > 0 && <div className="tooltip-row"><span>Cost</span><span className="tooltip-val">${inst.moneyCostPerHour}/h</span></div>}
          {inst.moneyIncomePerHour > 0 && <div className="tooltip-row"><span>Income</span><span className="tooltip-val">${inst.moneyIncomePerHour}/h</span></div>}
        </div>
      </div>
    );
  }
